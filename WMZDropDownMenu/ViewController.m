...
- (void)viewDidLoad {
    ...
    self.taData = @[@"展示全部属性和代理(仅用于参考用法)",@"闲鱼(三级联动参考)",
                    @"美团",@"淘宝",@"饿了么(自定义选中标题更改/输入框可点击)"
                    ,@"京东(查看更多,二级页面)",@"拼多多",@"简书",@"赶集网(标题关联相关)",
                    @"美团外卖(二级联动参考)",@"Boss直聘",@"唯品会(嵌套使用)",
                    @"一淘(放在tableViewHead上)",@"一淘(放在collectionView上+点击置顶)",
                    @"实际项目实例1",@"实际项目实例2(自适应cell宽度/cell对齐方式)",@"自定义展开视图"];
